...
function getUserGuess(grid, attempts) {
    ...
    let exitSequence = ["left", "left", "right", "right"];
    let exitIndex = 0;
    ...
    while (selectedWord.length < grid.length) {
        ...
        if (action === exitSequence[exitIndex]) {
            exitIndex++;
            if (exitIndex >= exitSequence.length) {
        ...
        // Scroll down automatically as the user navigates through the grid
        if (rowIndex >= maxRow) {
            rowIndex = maxRow - 1;
        }
        if (rowIndex < 0) {
...
function displayFeedback(feedback, grid) {
    for (let i = 0; i < feedback.positions.length; i++) {
        let pos = feedback.positions[i];
        for (let j = 0; j < grid[i].length; j++) {
            if (grid[i][j] === pos.letter) {
                if (pos.correct) {
                    grid[i][j] = "[" + to_string(grid[i][j]) + "]";
                } else if (pos.correct === false) {
                    grid[i][j] = "(" + to_string(grid[i][j]) + ")";
                } else {
                    grid[i][j] = to_string(grid[i][j]).toLowerCase();
                }
            }
        }
    }
